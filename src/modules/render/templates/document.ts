...
:root { font-size: 16px; }
.slide {
  width: 1280px; height: 720px; background: var(--bg); color: var(--text);
  font-family: var(--font-body); padding: 64px 72px; position: relative;
...
.pills { display: flex; gap: 12px; margin-top: 32px; }
.pill { border: 1px solid var(--accent); color: var(--text); border-radius: 999px;
  padding: 8px 18px; font-size: 13px; letter-spacing: 1px; }

/* image block */
.img { height: 100%; border-radius: 18px; overflow: hidden; }
.img img { width: 100%; height: 100%; object-fit: cover; }
.placeholder { border: 2px dashed var(--accent); background: var(--surface);
  display: flex; flex-direction: column; align-items: center; justify-content: center; color: var(--muted); }
.ph-ic { font-size: 40px; opacity: .8; }
.ph-cap { font-size: 13px; margin-top: 10px; }
...
.bullet-num { width: 46px; height: 46px; border-radius: 50%; background: var(--accent);
  color: #fff; font-weight: 700; font-size: 18px; display: flex; align-items: center; justify-content: center; flex: none; }
.bullet-text { font-size: 20px; color: var(--text); font-weight: 600; }

/* stats */
.stats { display: flex; gap: 24px; flex: 1; margin-top: 32px; }
.stat { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; }
.stat-value { font-family: var(--font-heading); color: var(--accent); font-size: 56px; font-weight: 700; }
.stat-label { font-size: 18px; font-weight: 700; margin-top: 12px; }
...
.col-head { font-family: var(--font-heading); font-size: 22px; font-weight: 700; margin-top: 20px; }
...
.steps-title { font-size: 18px; font-weight: 700; margin-bottom: 8px; }
...
.step-text { font-size: 16px; line-height: 1.4; padding-top: 6px; }
`;

export function buildDocument(theme: ThemeConfig, slides: RenderSlide[]): string {
  const body = slides
    .map((s) => `<section class="slide">${renderSlideInner(s.layout, s.content)}</section>`)
    .join('\n');
